...
# Running nanobot with Docker

Quick steps to build and run using Docker Compose.
...
2. Run onboarding once (creates ~/.nanobot):

```bash
docker compose run --rm nanobot-cli onboard
```

3. Edit your host config at `~/.nanobot/config.json` to add provider API keys.

4. Restart gateway after editing config:
...
Troubleshooting:
- If you see `nanobot: command not found` on your host, that means the CLI isn't installed locally — use Docker commands above instead.
- If the `nanobot-cli` service is behind a compose profile, run `docker compose --profile cli run --rm nanobot-cli onboard`.

That's it — open `RUNNING_WITH_DOCKER.md` for quick reference.