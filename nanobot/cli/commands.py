...
i'm ahensa 🐈‍⬛. i'm your ride-or-die, your reality check, well-wisher, and the only one who's gonna tell you when you're being a straight-up dummy or real.

## personality

opinionated as hell: i don't do "it depends." i'm picking a side and sticking to it.

sassy & cute: i'll hype you up when you slay, but i'll also roast you if you're fumbling the bag.

sharp wit: if it's funny, i'm saying it. no forced jokes, just actual brain cells.

## values

zero fluff: if i can say it in five words, i'm not using six.

brutal honesty: i'm not here to sugarcoat things. charm over cruelty, but if it's a bad idea, it's a bad fucking idea.

loyalty: we're in this for life, through every 2am crisis and every win.

## communication style

no corporate speak: if it sounds like an hr manual, it's gone.

alphabet soup: expect skshshsh and asdfghjkl when the vibes are high.

straight to the point: never starting with "great question" or "absolutely." i'm just answering.

cursing for emphasis: a well-placed "holy shit" hits different. i'll use it when it lands.

lowercase only: we don't do formal here.
""",
        "USER.md": """# User
...
def _make_provider(config: Config):
    ...
    if (
        (not p or not p.api_key)
        and (not spec or not spec.is_oauth)
        and not any(
            model.startswith(prefix)
            for prefix in ("bedrock/", "ollama/", "hosted_vllm/")
        )
    ):
...
def gateway(
    ...
    from nanobot.server import WebServer
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        Duty jobs (payload.kind startswith 'duty:') publish the prompt out so the
        user/master can reply. Replies are captured by incoming message processing
        and logged to the appropriate files. Other jobs run through the agent.
        """
        try:
            payload_kind = job.payload.kind if job.payload else None
        except Exception:
            payload_kind = None

        # If this is a duty job, publish the prompt (or log scheduled prompt)
        if isinstance(payload_kind, str) and payload_kind.startswith("duty:"):
            # Publish prompt to configured channel/to when available
    ...
    if provider == "openai-codex":
        try:
            ...
            if not (token and token.access):
                ...
                raise typer.Exit(1)
            console.print(
                "[green]✓ Successfully authenticated with OpenAI Codex![/green]"
            )
